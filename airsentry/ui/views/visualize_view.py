...
class _ChartWorker(QThread):
    """Generates charts in a background thread and emits chart paths."""

    charts_ready = Signal(list)   # list[Path]
    error        = Signal(str)
...
class VisualizeView(QWidget):
    ...
    The user selects a CSV or JSONL research dataset, clicks "Generate",
    and charts are rendered via the existing ``DatasetVisualizer`` backend.
    Each chart appears as a tab in the display area.
    """
    ...
    def _on_charts_ready(self, paths: list) -> None:
        ...
        self._status_label.setText(f"{len(paths)} chart(s) generated from {self._selected_path.name}")

        self._tab_widget.clear()

        for path in paths:
            path = Path(path)
            tab_title = self._CHART_TITLES.get(path.stem, path.stem.replace("_", " ").title())
        ...
        if not paths:
            self._status_label.setText(
                "No charts were generated. The dataset may be too small or missing required fields."
            )
